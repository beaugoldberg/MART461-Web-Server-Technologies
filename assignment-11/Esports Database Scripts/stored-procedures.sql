...
CREATE DEFINER=`root`@`localhost` PROCEDURE `spValidateUser`(in user_name varchar(50), in pwd varchar(50))
BEGIN
SELECT userID FROM users WHERE username = user_name AND pwd = pwd;
END$$
DELIMITER ;
